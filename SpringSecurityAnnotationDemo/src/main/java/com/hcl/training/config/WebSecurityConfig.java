...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication().withUser(users.username("vcp").password("vcp123").roles("GUEST"))
				.withUser(users.username("hcl").password("hcl123").roles("ADMIN"))
				.withUser(users.username("dba").password("dba123").roles("DBA"))
				.withUser(users.username("dot").password("dot123").roles("DOTNET"));
	}
	...
	protected void configure(HttpSecurity hs) throws Exception {
		hs.authorizeRequests()
				...
				.antMatchers("/dba-courses").hasAnyRole("DBA","ADMIN")
				.antMatchers("/net-courses").hasAnyRole("DOTNET","ADMIN")
